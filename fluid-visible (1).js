  ...
  // ============== Renderer / Camera (deferred init) ==============
  const gradientCanvas = componentEl;

  // We'll create renderer and heavy objects only once, on first start:
  let renderer = null;
  let camera = null;

  function createRendererIfNeeded() {
    if (renderer) return;
  ...
  // ============== Render targets & materials (deferred init) ==============
  let fluidTarget1, fluidTarget2, currentFluidTarget, previousFluidTarget;
  let fluidMaterial, displayMaterial;
  let geometry, fluidPlane, displayPlane;

  function initHeavyGL() {
    if (fluidMaterial) return; // already init
    ...
    fluidTarget1 = new THREE.WebGLRenderTarget(1, 1, rtOptions);
  ...
  // ============== GUI (dynamically imported if enabled) ==============
  ...
  function setRendererSize({ resetFluid = false } = {}) {
    ...
    if (fluidMaterial) fluidMaterial.uniforms.iResolution.value.set(width, height);
    if (displayMaterial) displayMaterial.uniforms.iResolution.value.set(width, height);
    if (fluidTarget1) fluidTarget1.setSize(width, height);
    if (fluidTarget2) fluidTarget2.setSize(width, height);
    if (resetFluid && fluidMaterial) requestSoftReset(config.softResetFrames, config.softResetStrength);
  }
  ...
  function updateMouseUniform(x, y) {
    ...
    if (fluidMaterial) fluidMaterial.uniforms.iMouse.value.set(mouseX, mouseY, prevMouseX, prevMouseY);
  }

  // We'll attach pointer listeners but they won't do heavy work until material exists.
  if (isFinePointer && hoverEnabled) {
    ...
    gradientCanvas.addEventListener('pointermove', onPointerMove);
    gradientCanvas.addEventListener('pointerleave', onPointerLeave);
  }
  ...
  function requestSoftReset(frames = 12, perFrameAttenuation = 0.15) {
    ...
    if (fluidMaterial) fluidMaterial.uniforms.uSoftReset.value = perFrameAttenuation;
  }

  // ============== CTA → colorIntensity ==============
  // Keep dynamic GSAP load: it won't run animations unless visible + init done
  ...
  if (ctaBtn) {
    ...
    ctaBtn.addEventListener('mouseenter', () => {
      if (window.gsap && displayMaterial) {
        gsap.to(displayMaterial.uniforms.uColorIntensity, {
          value: baseIntensity * mul,
          duration: parseFloat(ctaBtn.getAttribute('fc-fluid-gradient-transition-duration')) || 0.35,
          ease: ctaBtn.getAttribute('fc-fluid-gradient-transition-easing') || 'power1.out'
        });
      }
    });
  ...
  // ============== Main loop (visibility-controlled) ==============
  let frameCount = 0;
  let rafId = null;
  let lastFrameTime = 0;

  // throttling config (keeps cheaper)
  const THROTTLE_FPS = 30;
  const msPerFrame = 1000 / THROTTLE_FPS;

  function loop(now) {
    rafId = null;
    // 'now' can be undefined on first call in some browsers - normalize:
    ...
    if (!renderer || !fluidMaterial || !displayMaterial) {
  ...
  function startLoop() {
    // initialize heavy GL resources only when starting for the first time
    initHeavyGL();
    initGUI(); // safe to call repeatedly; initGUI guards inside
    setRendererSize({ resetFluid: false });
  ...
  function stopLoop() {
    ...
    // Optionally free GPU memory / detach render target textures,
    // but keep resources ready for quick restart instead of full tear-down.
    if (renderer) {
      // small optimization: clear the canvas to stop GPU compositing
      renderer.setRenderTarget(null);
      renderer.clear();
    }
  }
  ...
  // ============== Visibility control using IntersectionObserver ==============
  // Start rendering when component is at least 5% visible; stop when completely out.
  const IO_THRESHOLD = [0.05, 0.5]; // start when >5% visible; we still track deeper visibility
  ...
  if ('IntersectionObserver' in window) {
    ...
    io.observe(gradientCanvas);
    isObserving = true;
  } else {
    // no IO support: fallback to start immediately but still keep throttle
    startLoop();
  }

  // Optional: if the page uses single-page navigation and you removed Barba,
  // you might want to re-check visibility after navigation — that's outside scope
  // but you can call startLoop/stopLoop from your routing hooks.
